...
1. **Mean-value MC** (`mean_value_mc`) — базовий sample-mean, $I \approx (b-a) \cdot \overline{f(X)}$. Той самий метод, що в `task_02`. У літературі також зветься «crude MC» (синонім). Слугує еталоном для порівняння.
...
**VRF (variance reduction factor)** $= (\sigma_{\text{mv}}/\sigma_{\text{method}})^2$ — у скільки разів дисперсія методу менша за mean-value MC. `VRF = 10` означає, що метод при $N$ точок дає таку саму точність, як mean-value MC при $10N$ точок.
...
|---|---|---|---|
| **Mean-value MC** (sample-mean, він же «crude MC») | $O(1/\sqrt{N})$ | Майже ніколи в продакшені «в лоб». Базова цеглина, поверх якої будують решту. | numpy |
...
| Ціна опціона | Mean-value MC + antithetic + control variate, або QMC |
...
2. **Quasi-MC витіснив mean-value MC** у фінансах за останні 30 років — це фактично новий стандарт для розмірностей до ~10.
...
5. **Чистий mean-value MC сам по собі не використовують** — завжди комбінують з технікою зменшення дисперсії: antithetic + control variates як мінімум, або повноцінний QMC.
...
| --- | ---:| ---:| ---:| ---:|
| 1. Mean-value MC | 1.716308 | 0.001974 | 0.003837 | 1.0x |
...
| Метод | оцінка | abs err | std err | VRF |
| --- | ---:| ---:| ---:| ---:|
| 1. Mean-value MC | 0.249247 | 0.001415 | 0.002630 | 1.0x |
...
| Метод | оцінка | abs err | std err | VRF |
| --- | ---:| ---:| ---:| ---:|
| 1. Mean-value MC | 1.662889 | 0.009245 | 0.016085 | 1.0x |
...
На прикладі трьох функцій ми бачимо, що Монте-Карло — це **сімейство методів**, а не один алгоритм. Знання форми функції (монотонність, піки, симетрія, зміна знаку) дозволяє вибрати техніку, яка зменшить кількість необхідних точок у **десятки–сотні разів** при тій самій точності. Особливо це критично для багатовимірних задач, де mean-value MC стає непридатним.
